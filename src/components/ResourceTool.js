...
const ResourceTool = ({ pageInfo, errorMessage }) => {
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      >
        <Text>{`Disclaimer:\n\nQuantitative measurements are generally de-emphasized for POCUS applications. When quantitative measures are used, we are more closely approximating diagnostic level echocardiographic standards and thorough training is generally required. Those seeking more in depth review are urged to use the references provided.`}</Text>
      </Banner>
          ...
          >
            ...
            {section.table
              ? section.table.map((row) => (
                  <List.Accordion title={row.rowTitle} key={row.rowTitle}>
                    <View
                      style={{
            ...
            {section.calculator && (
              <List.Accordion title="Calculators">
                <Text
                  style={{ alignSelf: "center", color: "gray", fontSize: 12 }}
...
const Calculator = ({ id, formulaImage }) => {
  ...
  return (
    ...
    >
      <Text
        style={{ fontSize: 16, fontFamily: "Raleway-Medium", marginTop: 15 }}
      >
        {label}
      </Text>
      <Image
